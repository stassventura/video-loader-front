...
const fonts = ["Roboto", "Inter", "Montserrat", "LilitaOne"];
...
export default function Form({ className, ...props }: CardProps) {
  ...
  const fontValue = watch("fontName", fonts[0]);
  ...
  const onSubmit: SubmitHandler<Inputs> = (data) => {
    ...
    if (data.files.length === 1)
      successMessage = "Видео успешно отправлено на обработку.";
      ...
      <Card className={cn("w-full xs:w-[380px] mx-auto", className)} {...props}>
        ...
        <CardContent className="grid gap-4">
          <div className=" flex items-center space-x-4 rounded-md border p-4">
            ...
            <div className="flex-1 space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Отправить видео без звука
              </p>
            </div>
            <Switch
              value={String(pushValue)}
              onClick={() => setValue("push", !pushValue)}
            />
          </div>
          ...
          <div>
            ...
            <RadioGroup defaultValue={fonts[0]} id="font" className="mt-1">
              {fonts.map((font) => (
                <div
                  key={font}
                ...
                >
                  <RadioGroupItem
                    ...
                    id={font}
                    {...register("fontName")}
                    checked={font === fontValue}
                    onClick={() => setValue("fontName", font)}
                  />
                  <Label htmlFor={font} className="font-normal">
                    {font}
                  </Label>
                </div>
              ))}
          ...
          <div className="grid w-full max-w-lg items-center gap-1.5">
            ...
            {errors.files?.message && (
              <ErrorMessage text={errors.files?.message} />
            )}
          </div>
        </CardContent>
        <CardFooter>
